...
const SITE_URL = process.env.SITE_URL || 'https://jnutter.dev';
...
const CONTENT_TYPES = [
  { dir: 'src/content/blog/', urlPath: 'blog', excerptField: 'excerpt', imageField: 'image' },
  { dir: 'src/content/projects/', urlPath: 'projects', excerptField: 'description', imageField: 'thumbnail' },
];
...
async function main() {
  const allNewFiles = CONTENT_TYPES.flatMap(({ dir, urlPath, excerptField, imageField }) =>
    getNewFiles(dir).map(filePath => ({ filePath, urlPath, excerptField, imageField, dir }))
  );
  ...
  execSync('git add src/content/blog/ src/content/projects/');
  try {
